...
*********************************************************************/
// @stickbreaker 07/12/2018 modified to increase performance under Arduino-Esp32 
...
//@stickbreaker 07/12/2018 increase buffer by 1 byte to hold data marker in [-1], 
// adjust all offsets by +1 to account for this addition.  This change
// allows the use of block i2c transfer without individual read / write under ESP32 1.0.0-rc3 + 
...
// display Refresh bounding Box @stickbreaker 07/12/2018
static uint8_t boundX0=0,
  boundY0= 0, 
  boundX1= SSD1306_LCDWIDTH-1,
  boundY1= SSD1306_LCDHEIGHT-1; //all modified
...
void SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
  ...
  // update refresh bounding box
  if(x<boundX0) boundX0 = x;
  if(x>boundX1) boundX1 = x;
  if(y<boundY0) boundY0 = y;
  if(y>boundY1) boundY1 = y; 
  
  if((boundX1>SSD1306_LCDWIDTH-1)||(boundY1>SSD1306_LCDHEIGHT-1)){
    log_e("(%d,%d)-(%d,%d)",boundX0,boundY0,boundX1,boundY1);
    abort();
    }
  // x is which column
    switch (color)
    {
      case WHITE:   buffer[(x+ (y/8)*SSD1306_LCDWIDTH)+1] |=  (1 << (y&7)); break;
      case BLACK:   buffer[(x+ (y/8)*SSD1306_LCDWIDTH)+1] &= ~(1 << (y&7)); break;
      case INVERSE: buffer[(x+ (y/8)*SSD1306_LCDWIDTH)+1] ^=  (1 << (y&7)); break;
    }
  ...
  else
  {
    if (((boundX0 != 0)||(boundX1 !=SSD1306_LCDWIDTH-1)||(boundY0 !=0)||(boundY1!=SSD1306_LCDHEIGHT-1))&&
     (boundX0<=boundX1)){ // partial screen update
      uint16_t b = ((boundY0 / 8)*SSD1306_LCDWIDTH)+boundX0;
      uint16_t len = (boundX1 - boundX0) +2;
...
void SSD1306::clearDisplay(void) {
  ...
  // reset modified box to all modified.  
  boundX0 = 0;
  boundX1 = SSD1306_LCDWIDTH-1;
  boundY0 = 0;
  boundY1 = SSD1306_LCDHEIGHT-1;
...
void SSD1306::drawFastHLineInternal(int16_t x, int16_t y, int16_t w, uint16_t color) {
  ...
  // adjust bounding box
  if(x<boundX0) boundX0 = x;
  if((x+w-1)>boundX1) boundX1 = x+w-1; 
  if(y<boundY0) boundY0 = y;
  if(y>boundY1) boundY1 = y;
  ...
  register uint8_t *pBuf = (uint8_t*)&buffer[1];
...
void SSD1306::drawFastVLineInternal(int16_t x, int16_t __y, int16_t __h, uint16_t color) {
  ...
  if(x<boundX0) boundX0 = x;
  if(x>boundX1) boundX1 = x;
  if(__y<boundY0) boundY0 = __y;
  if((__y+__h-1)>boundY1) boundY1 = __y+(__h-1); 
